...
listings_details %>% 
  ...
  labs(title = "Price as a function of\ncategorical variables", y = "Mean_Price", subtitle = "Figure 2") +
     plotTheme() + theme(axis.text.x = element_text(angle = 45, hjust = 1))

```

```{r correlation plots, fig.height=5, fig.width=10}
st_drop_geometry(listings_details.sf) %>% 
  ...
  labs(title = "Price as a function of Numerical Variables", caption = "Figure 3") +
  plotTheme()
```
...
#Airbnb host provides entire home to tenant
...
do.call(grid.arrange,c(varList, ncol = 4, top = "Local Morans I statistics, Price", bottom = "Figure 5"))
```
...
``` {r few extra variable manipulations}
listings_details.sf <- 
  listings_details.sf %>% 
...
```{r correlation plots 1, fig.height=8, fig.width=15}
...
reg1 <- lm(price2 ~ ., data = st_drop_geometry(listings_details.sf) %>% 
                                 dplyr::select(price2, property_type, accommodates, bathrooms, bedrooms, neighbourhood, room_type, availability_365, number_of_reviews, minimum_nights, Entire_Home, markets_nn1, metrostops_nn2, students_nn2, wallart_nn3, playgrounds_nn2, metrolines_nn3, histbuild_nn3, monuments_nn3, restaurant_nn3, university_nn1, schools_nn2, retail_nn3, industrial_nn1, luxury, lagPrice, canal, expamen, expcodes, citycenterdesc, cheapcodes, pool, sig_cell, beds, review_scores_accuracy, review_scores_cleanliness, review_scores_rating, review_scores_communication, review_scores_location, review_scores_value, reviews_per_month, expamencat, expsummary, expdescrip, expneighbdesc, expneighwoclou, guests_included, cancellation_policy, size_of_group, expneighbors,hotspotlevel))
...
ErrorPlot1 <- ggplot() +
  geom_sf(data = neighborhoods_new.sf, fill = "grey40") +
  geom_sf(data = map_preds, aes(colour = q5(rentPrice.AbsError)),
          show.legend = "point", size = .01) +
  scale_colour_manual(values = paletteorngs,
                      labels=qBr(map_preds,"rentPrice.AbsError"),
                      name="Quintile\nBreaks") +
  labs(title="Absolute rental price errors \non the OOF set",
...
ErrorPlot2 <- ggplot() +
  ...
  geom_sf(data = map_preds, aes(colour = q5(PercentError)),
          show.legend = "point", size = .01, alpha= 0.7) +
...
``` {r district and neighborhood joins}


map_preds <- st_join(map_preds, districts.sf, left = TRUE)


map_preds <- st_join(map_preds, neighborhoods_new.sf, left = TRUE)

```


#Attempt at Neighborhood Maps & MAPE
...
st_drop_geometry(map_preds) %>%
  group_by(neighbourhood.y) %>%
    st_sf() %>%
    ggplot() + 
      geom_sf(data = map_preds, fill = "grey40") +
      geom_sf(aes(fill = meanMAE)) +
...
``` {r copy from DK Miami Code}
##############
# #Setting Up   
##############

## Controlling for Neighborhood ***OWN CHUNK****

## Neighborhood Fixed Effect
reg.mtracts <- lm(SalePrice ~ ., data = as.data.frame(miami.training) %>% 
                  dplyr::select(GEOID10, SalePrice, tjwf_nn1, 
                                Bed, parks_nn2, HHIncomeOpp, mschoolsLO_nn1,
                                mschoolsHI_nn1, ActualSqFt, MedHHinc,
                                Bath, LivingSqFt, bay_nn1,
                                mschoolsHI_nn3,pct_white, Price_PerSqFt))

miami.test.mtracts <-
  miami.test %>%
  mutate(Regression = "Tract Effects",
         SalePrice.Predict = predict(reg.mtracts, miami.test),
...
bothRegressions <- 
  rbind(
    dplyr::select(miami.test, starts_with("SalePrice"), Regression, GEOID10) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)),
    dplyr::select(miami.test.mtracts, starts_with("SalePrice"), Regression, GEOID10) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)))    

st_drop_geometry(bothRegressions) %>%
  gather(Variable, Value, -Regression, -GEOID10) %>%
...
#Baseline Regression | SalesPrice.AbsError 343576.5 | SalePrice.APE 0.6290319
#Tract Effects Regre | SalesPrice.AbsError 354291.1 | SalePrice.APE 0.8556324

##############
# #Setting Up   
##############

#Neighborhood Effect from Ken's Book 
bothRegressions %>%
  ...
  labs(title="Predicted sale price as a function of observed price",
       subtitle="Orange line represents a perfect prediction; Green line represents prediction", caption = "Figure 12") +
  plotTheme() + theme(plot.title = element_text(size = 18, colour = "black")) 
...
#MAPE from Ken's Book 
st_drop_geometry(bothRegressions) %>%
  group_by(Regression, GEOID10) %>%
  ...
  scale_fill_gradient(low = palette5[1], high = palette5[5],
                      name = "MAPE") +
  labs(title = "Mean test set MAPE by neighborhood", caption = "Figure 13") +
  mapTheme()
...
mtracts2 <- st_intersection(mtracts2, boundary)


grid.arrange(ncol = 2,
             ggplot() + geom_sf(data =mtracts2, aes(fill = percentWhite)) +
               scale_color_manual(values=c('#999999','#E69F00', '#56B4E9'), name="percentWhite") +
                labs(title = "% White") +
               mapTheme() + theme(legend.position="bottom"), 
             ggplot() + geom_sf(data = mtracts2, aes(fill = Median_Income)) +
               scale_color_manual(values=c('#999999','#E69F00', '#56B4E9'), name="Median_Income") +
               labs(title = "Income Context", caption = "Figure 14") +
               mapTheme() + theme(legend.position="bottom"))

 tidy(reg.mtracts) %>% 
   filter(str_detect(term, "GEOID10")) %>% 
   kable(title = "Table 6") %>% 
   kable_styling()
