...
######################
ggplot() +
  geom_sf(data = neighborhoods.sf, fill = "#2f4550") + 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
listings.sf$luxury <- ifelse(grepl("luxur",  ignore.case=TRUE, listings.sf$name), "yes", "no") 

listings.sf$canal <- ifelse(grepl("canal", ignore.case=TRUE, listings.sf$name), "yes", "no") 

listings.sf$expamen <- ifelse(grepl("view|terrace|spac|rooftop|loft|roof", ignore.case=TRUE, listings.sf$name), "yes", "no") 

listings.sf$expcodes <- ifelse(grepl("family|big|light|heart|large|design|pijp|jordaan", ignore.case=TRUE, listings.sf$name), "yes", "no") 

listings.sf$citycenterdesc <- ifelse(grepl("center|centre",ignore.case=TRUE, listings.sf$name), "yes", "no") 

listings.sf$cheapcodes <- ifelse(grepl("cozy|cosy|free|room|little|bed|garden|vondelpark", ignore.case=TRUE, listings.sf$name), "yes", "no") 
...
listings.sf <-
  listings.sf %>%
  mutate(
    metrostops_nn2 = nn_function(st_c(st_centroid(listings.sf)), st_c(st_centroid(metro_stops.sf)), 2))
...
listings.sf <-
  listings.sf %>%
  mutate(
    bar_nn2 = nn_function(st_c(st_centroid(listings.sf)), st_c(st_centroid(bar.sf)), 2))
...
listings.sf <-
  listings.sf %>%
  mutate(
    restaurant_nn3 = nn_function(st_c(st_centroid(listings.sf)), st_c(st_centroid(restaurant.sf)), 3))
...
listings.sf <-
  listings.sf %>%
  mutate(
    university_nn1 = nn_function(st_c(st_centroid(listings.sf)), st_c(st_centroid(university.sf)), 1))
...
listings.sf <-
  listings.sf %>%
  mutate(
    schools_nn2 = nn_function(st_c(st_centroid(listings.sf)), st_c(st_centroid(schools.sf)), 2))
...
listings.sf <-
  listings.sf %>%
  mutate(
    retail_nn3 = nn_function(st_c(st_centroid(listings.sf)), st_c(st_centroid(retail.sf)), 3))
...
listings.sf <-
  listings.sf %>%
  mutate(
    industrial_nn1 = nn_function(st_c(st_centroid(listings.sf)), st_c(st_centroid(industrial.sf)), 1))
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
listings.sf <- subset(listings.sf, price > 0)


reg1 <- lm(price ~ ., data = st_drop_geometry(listings.sf) %>% 
                                 dplyr::select(price, neighbourhood, room_type, availability_365, number_of_reviews, minimum_nights, Entire_Home, markets_nn1, metrostops_nn2, students_nn2, wallart_nn3, playgrounds_nn2, metrolines_nn3, histbuild_nn3, monuments_nn3, bar_nn2, restaurant_nn3, university_nn1, schools_nn2, retail_nn3, industrial_nn1, luxury))
...
```{r model reg testing}
#DK Playing with regression after new feature engineering. 

reg2 <- lm(price ~ ., data = st_drop_geometry(listings.sf) %>% 
                                 dplyr::select(price, neighbourhood, room_type, minimum_nights, number_of_reviews, reviews_per_month, calculated_host_listings_count, availability_365, City_Center, citycenterdesc, Entire_Home, luxury, canal, expamen, expcodes, cheapcodes, metrostops_nn2, metrolines_nn3,swim_nn1, wallart_nn3, markets_nn1, playgrounds_nn2, students_nn2, histbuild_nn3, monuments_nn3, bar_nn2, restaurant_nn3, university_nn1, schools_nn2, retail_nn3, industrial_nn1,lagPrice))
...
reg.training <- lm(price ~ ., data = st_drop_geometry(rents.training) %>% 
             dplyr::select(price, neighbourhood, availability_365, number_of_reviews, minimum_nights, Entire_Home, markets_nn1, metrostops_nn2, students_nn2, wallart_nn3, playgrounds_nn2, metrolines_nn3, histbuild_nn3, monuments_nn3, bar_nn2, restaurant_nn3, university_nn1, schools_nn2, retail_nn3, industrial_nn1, luxury))
...
rents.test <-
  ...
  mutate(Regression = "Baseline Regression",
         price.Predict = predict(reg1, rents.test),
...
ErrorTable <- 
  ...
  dplyr::summarize(Regression = "Baseline Regression",
                   MAE = mean(price.AbsError, na.rm = T), 
                   MAPE = mean(price.AbsError, na.rm = T) / mean(price, na.rm = T)) 
...
reg1_predict <- predict(reg.training, newdata = rents.test)

rmse.train <- caret::MAE(predict(reg.training), rents.training$price)
rmse.test <- caret::MAE(reg1_predict, rents.test$price)

preds.train <- data.frame(pred   = predict(reg.training),
                          actual = rents.training$price,
                          source = "training data")
preds.test  <- data.frame(pred   = reg1_predict,
                          actual = rents.test$price,
                          source = "testing data")
...
set.seed(717)
reg1.cv <- 
  train(price ~ ., data = st_drop_geometry(rents) %>% 
          dplyr::select(price, neighbourhood, availability_365, number_of_reviews, minimum_nights, Entire_Home, markets_nn1, metrostops_nn2, students_nn2, wallart_nn3, playgrounds_nn2, metrolines_nn3, histbuild_nn3, monuments_nn3, bar_nn2, restaurant_nn3, university_nn1, schools_nn2, retail_nn3, industrial_nn1, luxury), 
        method = "lm", 
