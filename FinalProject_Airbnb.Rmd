...
st_drop_geometry(listings_details.sf) %>% 
  ...
  gather(Variable, Value, -price2) %>% 
  ggplot(aes(Value, price2)) +
  geom_point(shape = 16, size = 1.5, color= "#72b7cd", alpha = 0.5) + geom_smooth(method = "lm", se=F, colour = "#E86E23") +
...
st_drop_geometry(listings_details.sf) %>% 
  ...
  gather(Variable, Value, -price2) %>% 
  ggplot(aes(Value, price2)) +
  geom_point(shape = 16, size = 1.5,color= "#cde1b1", alpha = 0.5) + geom_smooth(method = "lm", se=F, colour = "#E86E23") +
  facet_wrap(~Variable, ncol = 3, scales = "free") +
...
st_drop_geometry(listings_details.sf) %>% 
  dplyr::select(price2, room_type) %>%
  filter(price2 <= 1000000) %>%
  gather(Variable, Value, -price2) %>% 
  ggplot(aes(Value, price2)) +
  geom_point(shape = 16, size = 3,color= "#72b7cd", alpha = 0.5) + geom_smooth(method = "lm", se=F, colour = "#E86E23") +
  facet_wrap(~Variable, ncol = 3, scales = "free") +
  labs(title = "Price as a function of Nearest Three Rental Prices", caption = "Figure 3") +
  plotTheme()
